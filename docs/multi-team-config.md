...
The Metal Credential Sync supports configuring multiple secret engines within a single Vault backend. This allows different teams or environments to sync BMC credentials to separate Vault mount paths with different path templates and label selectors.
...
- Global label: `metal-credential-sync.metal.ironcore.dev/sync`
- Engine label: `team=a`
...
vault write auth/kubernetes/role/metal-credential-sync \
  bound_service_account_names=metal-credential-sync \
...
3. **Check operator logs**: Look for sync decisions
   ```bash
   kubectl logs -n metal-credential-sync-system -l control-plane=controller-manager
   ```
