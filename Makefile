...
KIND_CLUSTER ?= metal-credential-sync-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name metal-credential-sync-builder
	$(CONTAINER_TOOL) buildx use metal-credential-sync-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm metal-credential-sync-builder
	rm Dockerfile.cross
...
check-license: addlicense ## Check that every file has a license header present.
	find . -path ./third_party -prune -o -name '*.go' -exec "$(ADDLICENSE)" -check -c 'CobaltCore authors' {} +
